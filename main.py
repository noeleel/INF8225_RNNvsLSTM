...
import torch
import sklearn
from datasetNLTK import word_to_ix, tag_to_ix,train_data,prepare_sequence, test_set, validation_set, t_fin_dict,t_init, short_test_set, long_test_set
from modelGRU import SimpleGRU, DoubleGRU, BiGRU
from modelLSTM import LSTMTagger, LSTMTagger_bidir_2layers, LSTMTagger_bidir
from modelRNN import SimpleRNN, DoubleRNN, BidirectionalRNN

from parameters import EMBEDDING_DIM, HIDDEN_DIM, LEARNING_RATE, N_EPOCHS, loss_function
...
with open('Resultats.txt', 'w') as f:
    with redirect_stdout(f):
        ...
        List_model = [LSTMTagger(EMBEDDING_DIM, HIDDEN_DIM, len(word_to_ix), len(tag_to_ix)), 
                      LSTMTagger_bidir_2layers(EMBEDDING_DIM, HIDDEN_DIM, len(word_to_ix), len(tag_to_ix)), 
                      LSTMTagger_bidir(EMBEDDING_DIM, HIDDEN_DIM, len(word_to_ix), len(tag_to_ix)),
                      SimpleRNN(EMBEDDING_DIM, HIDDEN_DIM, len(word_to_ix), len(tag_to_ix)),
                      ...
                      BiGRU(EMBEDDING_DIM, HIDDEN_DIM,len(word_to_ix),  len(tag_to_ix))]
